...
<body>
  <div style="display: flex; align-items: center;">
    <span class="time" style="font-size: 15px">Nota ID: {{$riwayat_nota->id_pesanan}}</span>
  </div>
  <div class="card-body">
    <table class="table">
      <thead>
        ...
        <tr id="row_{{$pesanan->id}}">
          ...
          <td>
            <div style="width: 100%;">
              <span style="width: 10%;">
                @php
                $gambar = $pesanan->data_produk->foto ?? "0";
                if ($gambar == "0"){
                    $path = public_path('img/image_not_available.png');
                }
                else {
                    $path = public_path('img/produk/thumbnail/300x300/').$pesanan->data_produk->foto;
                    if (File::exists($path)) {
